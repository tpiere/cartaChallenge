...
describe("App", () => {
  ...
  it('renders a main heading', () => {
    ...
    expect(screen.getByRole('heading')).toHaveTextContent('Find nearby places of interest')
  });

  it('displays nearby places', async () => {
    nearbyPlacesService.getNearbyPlaces = jest.fn().mockResolvedValue(
      [
        { id: 1, name: 'place 1', formattedAddressParts: ['111 1st street', 'Seattle', 'WA'] }
      ]
    );
      ...
      fireEvent.change(getByLabelText('Enter a location'), { target: { value: 'Seattle, WA' } })
